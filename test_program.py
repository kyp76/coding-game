...
def test_batman_take_a_gird():
    ...
    # white_gird = bat.get()
    white_gird = bat.get()
    bat.set(new_gird= white_gird, new_direction= "R", new_bat_x = 4, new_bat_y=
            4)
...
def test_if_jump_return_is_expected_value_on_white_bat():
    bat = Batman(width=4, height=4, bat_x=3, bat_y=2, direction="R")
    white_gird = bat.get()
    #bat.set(white_gird)
    value = bat.jump()
    assert (2, 3) == white_gird[3]
...
#        bat.set(white_gird)
#        value = bat.jump()
#        bat.set("U", white_gird, new_bat_x= value[0], new_bat_y= value[1])
